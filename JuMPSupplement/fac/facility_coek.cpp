...
int main(int argc, char *argv[])
{
    ...
    // Create variables
    coek::Variable d = model.add_variable(0, COEK_INFINITY, 1.0);
    ...
    for (int i = 0; i <= G; i++) {
        ...
        for (int j = 0; j <= G; j++) {
            ...
            for (int f = 0; f < F; f++) {
                z[i][j].push_back(
                    model.add_variable(0, 1, 0, true, false)
                );
            }
        }
    }
    ...
    for (int i = 0; i <= G; i++) {
        ...
        for (int j = 0; j <= G; j++) {
            ...
            for (int f = 0; f < F; f++) {
                s[i][j].push_back(
                    model.add_variable(0, COEK_INFINITY, 0)
                );
            }
        }
    }
    ...
    for (int i = 0; i <= G; i++) {
        ...
        for (int j = 0; j <= G; j++) {
            ...
            for (int f = 0; f < F; f++) {
                ...
                for (int k = 0; k < 2; k++) {
                    r[i][j][f].push_back(
                        model.add_variable(-COEK_INFINITY, COEK_INFINITY, 0)
                        );
                }
            }
        }
    ...
    for (int i = 0; i <= G; i++) {
        for (int j = 0; j <= G; j++) {
            for (int f = 0; f < F; f++) {
                ...
                model.add_constraint(r[i][j][f][0] == (1.0*i)/G - y[f][0]);
                model.add_constraint(r[i][j][f][1] == (1.0*j)/G - y[f][1]);
                model.add_constraint(r[i][j][f][0]*r[i][j][f][0] + 
                                 r[i][j][f][1]*r[i][j][f][1] <= 
                                 s[i][j][f]*s[i][j][f]);
            }
        }
